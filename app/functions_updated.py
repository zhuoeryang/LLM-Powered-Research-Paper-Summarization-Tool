...
def create_vectorstore(documents, embedding_function, file_name="default"):
    # Deduplicate chunks if needed
    unique_chunks = []
    seen_ids = set()
    ...
    vectorstore = FAISS.from_documents(documents=unique_chunks, embedding=embedding_function)
    return vectorstore
...
def load_vectorstore(file_name, api_key, vectorstore_path="db"):
    ...
    path = os.path.join(vectorstore_path, cleaned_name)
    
    return FAISS.load_local(folder_path=path, embeddings=embedding_function)
